...
export class VirtualGPS {
  ...
  step(){if(!this.enabled||!this.position)return;if(!this.target){if(Date.now()-this.position.timestamp>=1000)this.set({...this.position,speed:0});return}const speed={walk:4,brisk:6,run:10}[this.mode]||4,b=bearingTo(this.position,this.target),d=Math.hypot((this.position.lat-this.target.lat)*111320,(this.position.lng-this.target.lng)*90000);if(d<1){this.target=null;this.set({...this.position,speed:0});return}this.set({...destination(this.position,b,Math.min(d,speed/3.6/4)),speed:speed/3.6});}
