...
assert(JSON.stringify(WEAPON_IDS)===JSON.stringify(['balanced','long','heavy','light']),'Four weapons must be registered in display order');
...
for(const token of ['p.nodes.length','weapon.damageMultiplier','WEAPON_IDS[Math.floor(Math.random()','state=\'countdown\'','countdownNumber','setPointerCapture','pointercancel','lostpointercapture'])assert(gameSource.includes(token),'Missing upgraded game behavior: '+token);
